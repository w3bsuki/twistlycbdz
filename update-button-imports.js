...
// Script to update Button imports to use the standard path @/components/ui/button
// Run with: node update-button-imports.js
...
const OLD_IMPORT_PATTERN = /import\s+(?:{[^}]*?Button[^}]*?})?\s*from\s+['"]@\/components\/common\/ui\/button['"]/g;
const NEW_IMPORT = "import { Button } from '@/components/ui/button'";

// If we need to extract other imports that might be in the same line
const EXTRACT_OTHER_IMPORTS = /import\s+{([^}]*?Button[^}]*?)}\s*from\s+['"]@\/components\/common\/ui\/button['"]/;
  ...
  try {
    ...
    if (OLD_IMPORT_PATTERN.test(content)) {
      ...
      let newContent = content.replace(OLD_IMPORT_PATTERN, (match) => {
        // If there are other components being imported alongside Button
        ...
        if (extractMatch && extractMatch[1]) {
          ...
          const otherImports = imports.filter(imp => !imp.includes('Button') && imp !== '');
          
          if (otherImports.length > 0) {
            // Keep the other imports for the common/ui path
            return `import { ${otherImports.join(', ')} } from '@/components/common/ui/button';\n${NEW_IMPORT}`;
          }
        }
        return NEW_IMPORT;
      });
...
// Main function
async function main() {
  console.log('Starting Button import updates...');
